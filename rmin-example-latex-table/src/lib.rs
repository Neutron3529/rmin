...
mod column;
pub use column::{ItemFn, Column, ColumnAlign, Columns};
...
fn _print(
    ...
    let columns = if columns.len() < 2 {
        ...
        [columns[0], columns[1]].map(|x|str::from_utf8(x.data()).expect("the given column format could not be converted into utf8").to_string())
    };
    let column_names : Vec<_> = column_names.data().iter().map(|x|str::from_utf8(x.data()).expect("the given column name could not be converted into utf8").to_string()).collect();
    ...
    if roundings.len()==1 {
        ...
        a.content.data_columns.columns.iter_mut().zip(roundings.iter()).for_each(|(x,&y)|x.ifn.rounding=y as u32)
    }
    a.content.data_columns.set_names(column_names);
    
    a.content.data = Data::from(data.data().iter());
    fn modify<'a,T:'a>(a:&'a mut Table, format:impl ExactSizeIterator<Item=&'a T>, modify: impl FnMut((&'a mut Data<f64>, &'a T))){
        if format.len()>0 {
            if format.len() != a.content.data.len() {
                panic!("format length {} is not equals to data length {}", format.len(), a.content.data.len())
            } else {
                a.content.data.iter_mut().zip(format).for_each(modify)
            }
        }
    }
    modify(&mut a,bold.data().iter(), |(data,&item)|data.as_bold = item == 1);
