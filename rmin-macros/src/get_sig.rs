...
pub fn get_sig(ret:&mut TokenStream, iter:&mut impl Iterator<Item=TokenTree>)->(String,Vec<[String; 2]>) {
    let Some(Group(g)) = iter.next() else {
        panic!("need a parameter list after name");
    };
    let gparam=g.stream().into_iter().collect::<Vec<_>>().split(|x|if let Punct(y)=x {y.as_char()==','} else {false}).filter_map(|x|{
        let def = x.splitn(2,|x|if let Punct(y)=x {y.as_char()==':'} else {false}).collect::<Vec<_>>();
        if let [name,ty] = def[..] {
